...
template< typename TPosition, typename TSize >
void Ardutils::Rectangle< TPosition, TSize >::Offset(const Vector2<TPosition> & vector)
{
	this->X += vector.X;
	this->Y += vector.Y;
}
...
template< typename TPosition, typename TSize >
bool Ardutils::Intersects(const Rectangle<TPosition, TSize> & left, const Rectangle<TPosition, TSize> & right)
{
	return !
		(
